...
function jobs_load ()
{
	...
	$job->text_tasks_set ('Betreuung und Ausbau von Bestandskunden');
	...
	$job = new job ('Kauffrau /-mann für Bürokommunikation', 'Du lebst für gute Organisation und hast Lust uns in unseren Arbeitsabläufen tatkräftig zu unterstützen?');
	$job->url_set ('/karriere/job-kaufmann-frau-fuer-buerokommunikation-potsdam');
	$job->quickinfo_set ('Potsdam · unbefristet · (m/w/d)');
	$job->text_intro_set ('Du brennst für Organisation und Planung und wärst bereit uns im Alltagsgeschäft tatkrätig zu unterstützen? Dann haben wir einen Job für dich!');
	...
	$job->text_tasks_set ('Zuarbeit bei Onboardings und Kunden-Projekten');
	...
	$job->text_requirements_set ('Gute technisches Auffassungsgabe und der Wille, technische Sachverhalte zu begreifen');
	$job->text_requirements_set ('Eigenständige, proaktive und sorgfältige Arbeitsweise');
	$job->text_requirements_set ('Sicherer Umgang mit den gängigen Programmen wie Word, Excel und Power Point');
	...
	$job->text_tasks_set ('Ideen und technisches Wissen zur Weiter- und Neuentwicklung von Produkten');
	...
	$job->text_requirements_set ('Grundkenntnisse in den Bereichen VoIP und SIP sowie Betriebssystemen und Arbeit mit der Browser-Console');
	...
	$job->statement_set (new job_statement ('/assets/images/photos/brian_miculcy.jpg', 'Brian Miculcy', 'Head of IT', 'Zukunftstechnologien nutzen und zusammen mit Kunden kreative Lösungen schaffen. Dabei jederzeit ein starkes Team hinter einem wissen.'));
	$job->icon_set ('/assets/images/icons_svg/job_02.svg');
	$jobs[] = $job;

	$job = new job ('IT-Support', 'Du hast ein solides Arsenal technisches Wissen und kannst Partner, Kunden und IT-Mitarbeiter umfassend betreuen?');
	$job->url_set ('/karriere/job-it-support-potsdam');
	$job->quickinfo_set ('Potsdam · unbefristet · (m/w/d)');
	$job->text_intro_set ('Du hast eine technische Ausbildung erfolgreich absolviert und kannst analytisch-strukturiert denken?	Du besitzt eine hohe Auffassungsgabe für komplexe Prozesse und arbeitest gerne im Team? Dann bist du bei uns genau richtig!');
	...
	$job->text_requirements_set ('Optimal wären Grundkenntnisse in den Bereichen VoIP und SIP sowie Betriebssystemen und Arbeit mit der Browser-Console ');
	...
	$job = new job ('Fachinformatiker*in', 'Du bist Fachinformatiker*in mit Schwerpunkt Entwicklung oder Systemintegration und hast Lust uns mit deiner Expertise zu unterstützen?');
	...
	$job = new job ('Webdesigner*in', 'HTML-Programmier-Dienstleistungen und Grafikdesign sind dein tägliches Handwerk? Dann bist du bei uns genau richtig!');
	...
	$job->text_intro_set ('HTML-Programmier-Dienstleistungen und Grafikdesign sind dein tägliches Handwerk? Dann bist du bei uns genau richtig!​​');
	...
	$job->text_tasks_set ('Erstellung Online-Werbemitteln und Landingpages');
	$job->text_requirements_set ('Abgeschlossenes Studium oder Ausbildung in Grafikdesign, Mediendesign, Kommunikationsdesign oder vergleichbarer Qualifikation');
...
function jobs_benefits_load ()
{
	...
	$jobs_benefit->text_set ('Uns allen ist der Wert unserer Arbeit besonders wichtig. Wir wollen, dass unsere Arbeit einen Einfluss hat. Das bedeutet für uns: Fokussiertes, eﬀizientes Arbeiten, das sich an Ergebnissen orientiert. Aber auch pünktlich Feierabend machen zu können und am Wochenende frei zu haben.');
	$jobs_benefits[] = $jobs_benefit;
