...
function jobs_load ()
{
	...
	*/
	...
	$job->text_description_long_set ('Klingt nach einer lohnenden Investition? Dann verabrede dich jetzt mit Marian: <a href="https://outlook.office365.com/owa/calendar/G28574a2b3be345b4b8c268ed1ddc5cd9@comdesk.de/bookings/" target="_blank">Zur Terminbuchung</a>');
	$job->text_tasks_set ('Entwicklung von responsiven und benutzerfreundlichen Webanwendungen');
	$job->text_tasks_set ('Enge Zusammenarbeit mit weiteren Frontend-, sowie den Backend-Entwicklern');
...
<b>Gibt es gar keine Mindestvoraussetzungen?</b> Doch! Du solltest natürlich Lust auf Backend Entwicklung haben, PHP-Affinität sowie Erfahrung mit Schnittstellen und Datenbanken mitbringen – alles weitere ist verhandelbar. :)');
	...
	*/
	...
	$job->quickinfo_set ($job->location_get().' · unbefristet · (m/w/d)');
	$job->text_intro_set ('Keine Lust auf lange Entscheidungswege, veraltete Arbeitsstrukturen und Anwesenheitspflicht? Du suchst einen verantwortungsvollen Job, in dem du wirklich Mehrwert stiftest, und wünschst dir ein Umfeld, in dem du deine Ideen schnell und unbürokratisch umsetzen kannst? Du willst New Work nicht nur als Buzzword in einer Jobanzeige lesen, sondern den Job deinen Interessen und Bedürfnissen anpassen und dich auf Wunsch für eine 4-Tage-Woche, remote oder hybrid work entscheiden können? Dann lohnt es sich, jetzt weiterzulesen!');
	$job->text_description_long_set ('Du planst und wählst neue Rechenzentrums-Hardware aus und bist für das Monitoring mit CheckMk verantwortlich. Du kümmersz dich zudem um die Datensicherung und -wiederherstellung sowie das Testing. Bei Bedarf betreust du auch die Netzwerkinfrastruktur und Datenbank.<br /><br />Wir bieten ein wettbewerbsfähiges Gehaltspaket und eine angenehme Arbeitsumgebung in einem dynamischen und schnelllebigen Unternehmen. Wenn Sie eine Leidenschaft für Serverinfrastrukturen und Storage Systeme haben und eine herausfordernde Position suchen, dann bewerben Sie sich jetzt als Serverinfrastruktur- und Storage Engineer.');
	...
	$job->text_requirements_set ('Kenntnisse mit Windows (wünschenswert, aber kein Must-have)');
	$job->text_requirements_set ('Kenntnisse mit Virtualisierung wie Proxmox (LXC / KVM) und ggf. Hyper-V');
	$job->text_requirements_set ('Kenntnisse mit Firewall und Netzwerk Technik (wünschenswert, aber kein Must-have)');
	$job->text_requirements_set ('Kenntnisse mit Datenbank Lösungen wie MariaDB / MySQL (wünschenswert, aber kein Must-have)');
	$job->text_requirements_set ('Kenntnisse mit CheckMK oder ähnlichen Lösungen');
	$job->text_requirements_set ('Kenntnisse mit VoIP (wünschenswert, aber kein Must-have)');
	$job->text_requirements_set ('Kenntnisse mit PHP und SQL (wünschenswert, aber kein Must-have)');
	$job->text_requirements_set ('Berufsausbildung, Bachelor- oder Masterabschluss in Informatik oder einem verwandten Bereich');
...
$job = new job ('Management', '(Junior) Finance Specialist (w/m/d) I 20 bis 40 Std. I 80% Home Office möglich', 'Home Office, 30 Tage Urlaub, keine Überstunden, Weiterbildungsbudget... als Finance Specialist (w/m/d) bei CallOne darfst du dich auf tolles Gesamtpaket freuen!');
	...
	$job->text_description_long_set ('Dabei sind wir kein hippes, überbewertetes Startup, das Geld verbrennt, sondern ein <b>profitables, mittelständisches Unternehmen</b>, das sich dank dem Engagement und der (Tech-)Leidenschaft unserer MitarbeiterInnen in den letzten 13 Jahren mit seinen innovativen und zugleich zuverlässigen Lösungen einen Namen in der Branche gemacht hat. So sind wir beispielsweise das erste Unternehmen, das “über Nacht” eine funktionierende, ChatGPT-basierte Voice Lösung entwickelt und gelauncht hat, die die Service Center Verantwortlichen bei uns Schlange stehen lässt.');
	$job->text_description_long_set ('<b>Nun suchen wir DICH, um die Erfolgsgeschichte fortzusetzen!</b>');
