...
class Carddeck{constructor(t){this.deck=t,this.currentCard=0,this.speed=parseInt(this.deck.dataset.speed)||2e3,this.allCards=Array.from(this.deck.querySelectorAll(".carddeck__card")),this.cardCount=this.allCards.length,this.duplicateCards(),this.wrapCardContent(),this.cardSize=this.getCardSize(),this.cards=this.initCards(),this.indicators=this.initIndicators(),this.deckInterval=setInterval(this.switch.bind(this),this.speed),respondToVisibility(this.deck,t=>{this.resizeDeck()}),window.addEventListener("resize",(t=>{this.resizeDeck()}).bind(this))}resizeDeck(){this.cardSize=this.getCardSize(),this.cards.style.height=this.cardSize+"px",this.allCards.forEach(t=>t.style.height=this.cardSize+"px"),this.indicators.style.height=this.cardSize+"px"}wrapCardContent(){this.allCards.forEach(t=>{t.innerHTML="<div>"+t.innerHTML+"</div>"})}initCards(){let t=document.createElement("div");return t.classList.add("carddeck__cards"),t.style.height=this.cardSize+"px",this.allCards.forEach(t=>t.style.height=this.cardSize+"px"),this.allCards.forEach((e,d)=>{0==d?e.classList.add("carddeck__card--top"):1==d?e.classList.add("carddeck__card--center"):2==d?e.classList.add("carddeck__card--bottom"):e.classList.add("carddeck__card--hidden"),t.appendChild(e)}),this.deck.innerHTML="",this.deck.appendChild(t),t}duplicateCards(){this.allCards.length<4&&(this.allCards.forEach(t=>{this.allCards.push(t.cloneNode(!0))}),this.allCards.length<4&&this.duplicateCards())}initIndicators(){let t=document.createElement("div");t.classList.add("carddeck__indicators"),t.style.height=this.cardSize+"px";for(let e=0;e<this.cardCount;e++){let d=document.createElement("div");d.classList.add("carddeck__indicator"),0==e&&d.classList.add("carddeck__indicator--active"),t.appendChild(d)}return this.deck.appendChild(t),t}getCardSize(){let t=0;return this.allCards.forEach(e=>{e.style.height="",e.classList.remove("carddeck__card--messured"),e.offsetHeight>t&&(t=e.offsetHeight),e.classList.add("carddeck__card--messured")}),t}switch(){this.currentCard++;let t=this.allCards.length;this.currentCard>=t&&(this.currentCard=0);let e=((this.currentCard+2)%t+t)%t,d=this.allCards[e];d.classList.remove("carddeck__card--hidden");let s=this.cards.querySelector(".carddeck__card--bottom"),a=this.cards.querySelector(".carddeck__card--center"),c=this.cards.querySelector(".carddeck__card--top");this.cards.appendChild(d),d.classList.add("carddeck__card--bottom"),s.classList.toggle("carddeck__card--bottom"),s.classList.toggle("carddeck__card--center"),a.classList.toggle("carddeck__card--center"),a.classList.toggle("carddeck__card--top");let r=new CustomEvent("testEvent",{detail:{visible:!0}});a.querySelectorAll("*").forEach(t=>{t.dispatchEvent(r)}),c.classList.toggle("carddeck__card--top"),c.classList.toggle("carddeck__card--fade"),setTimeout(()=>{c.classList.remove("carddeck__card--fade"),c.classList.add("carddeck__card--hidden")},610),this.indicators.querySelector(".carddeck__indicator--active").classList.remove("carddeck__indicator--active");let i=this.currentCard;this.currentCard>=this.cardCount&&(i-=this.cardCount),1==this.cardCount&&(i=0),this.indicators.querySelectorAll(".carddeck__indicator")[i].classList.add("carddeck__indicator--active")}}document.addEventListener("DOMContentLoaded",t=>{document.querySelectorAll(".carddeck").forEach(t=>{new Carddeck(t)})});
...
class CountingNumbers{constructor(t){this.element=t,this.speed=1e3,this.suffix=this.element.dataset.suffix||"",this.start=parseInt(this.element.dataset.start),this.end=parseInt(this.element.dataset.end),this.current=this.start,this.step=this.calculateStep(),this.running=!1,this.element.addEventListener("testEvent",t=>{this.reset(),this.isInView()}),window.addEventListener("scroll",this.isInView.bind(this))}isInView(){this.position=this.element.getBoundingClientRect(),this.position.top>=0&&this.position.bottom<=window.innerHeight?this.running||(this.update(),this.count(),this.running=!0):this.reset()}reset(){this.running=!1,this.current=this.start,this.update()}calculateStep(){return Math.abs(this.start-this.end)/(this.speed/30)}count(){this.start<this.end?(this.current+=this.step,this.current>this.end&&(this.current=this.end)):(this.current-=this.step,this.current<this.end&&(this.current=this.end)),this.update(),this.current!=this.end?setTimeout(this.count.bind(this),1e3/30):this.running=!1}update(){this.element.textContent=Math.round(this.current)+this.suffix}}document.addEventListener("DOMContentLoaded",t=>{document.querySelectorAll(".counting-number").forEach(t=>{new CountingNumbers(t)})});
