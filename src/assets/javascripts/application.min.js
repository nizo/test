...
class Carddeck{constructor(t){this.deck=t,this.currentCard=0,this.speed=parseInt(this.deck.dataset.speed)||2e3,this.allCards=Array.from(this.deck.querySelectorAll(".carddeck__card")),this.cardCount=this.allCards.length,this.duplicateCards(),this.wrapCardContent(),this.cardSize=this.getCardSize(),this.cards=this.initCards(),this.indicators=this.initIndicators(),respondToVisibility(this.deck,t=>{t?(this.resizeDeck(),this.deckInterval=setInterval(this.switch.bind(this),this.speed)):clearInterval(this.deckInterval)}),window.addEventListener("resize",(t=>{this.resizeDeck()}).bind(this))}resizeDeck(){this.cardSize=this.getCardSize(),this.cards.style.height=this.cardSize+"px",this.allCards.forEach(t=>t.style.height=this.cardSize+"px"),this.indicators.style.height=this.cardSize+"px"}wrapCardContent(){this.allCards.forEach(t=>{t.innerHTML="<div>"+t.innerHTML+"</div>"})}initCards(){let t=document.createElement("div");return t.classList.add("carddeck__cards"),t.style.height=this.cardSize+"px",this.allCards.forEach(t=>t.style.height=this.cardSize+"px"),this.allCards.forEach((d,e)=>{0==e?d.classList.add("carddeck__card--top"):1==e?d.classList.add("carddeck__card--center"):2==e?d.classList.add("carddeck__card--bottom"):d.classList.add("carddeck__card--hidden"),t.appendChild(d)}),this.deck.innerHTML="",this.deck.appendChild(t),t}duplicateCards(){this.allCards.length<4&&(this.allCards.forEach(t=>{let d=t.cloneNode(!0);this.allCards.push(d)}),this.allCards.length<4&&this.duplicateCards())}initIndicators(){let t=document.createElement("div");t.classList.add("carddeck__indicators"),t.style.height=this.cardSize+"px";for(let d=0;d<this.cardCount;d++){let e=document.createElement("div");e.classList.add("carddeck__indicator"),0==d&&e.classList.add("carddeck__indicator--active"),t.appendChild(e)}return this.deck.appendChild(t),t}getCardSize(){let t=0;return this.allCards.forEach(d=>{d.style.height="",d.classList.remove("carddeck__card--messured"),d.offsetHeight>t&&(t=d.offsetHeight),d.classList.add("carddeck__card--messured")}),t}switch(){this.currentCard++;let t=this.allCards.length;this.currentCard>=t&&(this.currentCard=0);let d=((this.currentCard+2)%t+t)%t,e=this.allCards[d];e.classList.remove("carddeck__card--hidden");let a=this.cards.querySelector(".carddeck__card--bottom"),s=this.cards.querySelector(".carddeck__card--center"),c=this.cards.querySelector(".carddeck__card--top");this.cards.appendChild(e),e.classList.add("carddeck__card--bottom"),a.classList.toggle("carddeck__card--bottom"),a.classList.toggle("carddeck__card--center"),s.classList.toggle("carddeck__card--center"),s.classList.toggle("carddeck__card--top"),c.classList.toggle("carddeck__card--top"),c.classList.toggle("carddeck__card--fade"),setTimeout(()=>{c.classList.remove("carddeck__card--fade"),c.classList.add("carddeck__card--hidden")},610),this.indicators.querySelector(".carddeck__indicator--active").classList.remove("carddeck__indicator--active");let r=this.currentCard;this.currentCard>=this.cardCount&&(r-=this.cardCount),1==this.cardCount&&(r=0),this.indicators.querySelectorAll(".carddeck__indicator")[r].classList.add("carddeck__indicator--active")}}document.addEventListener("DOMContentLoaded",t=>{document.querySelectorAll(".carddeck").forEach(t=>{new Carddeck(t)})});
