...
class Typing{constructor(t){this.element=t,this.texts=this.element.dataset.texts.split("|"),this.colors=Array(this.texts.length),this.colors.fill("currentColor"),this.element.dataset.colors&&(this.colors=this.element.dataset.colors.split("|")),this.links=Array(this.texts.length),this.links.fill(null),this.element.dataset.links&&(this.links=this.element.dataset.links.split("|")),this.currentText=0,this.textIndex=0,this.speed=parseInt(this.element.dataset.speed)||50,this.keepTime=2e3,this.element.classList.contains("typing-text--block")&&this.setElementHeight(),this.element.textContent="",this.element.style.color=this.colors[this.currentText],this.write()}setElementHeight(){let t=0;this.texts.forEach(e=>{this.element.textContent=e,this.element.offsetHeight>t&&(t=this.element.offsetHeight)}),this.element.textContent="",setTimeout(()=>{this.element.style.height=t+"px"},30)}write(){let t=this.texts[this.currentText],e=this.links[this.currentText],s=t.length;this.textIndex<s?(this.element.textContent+=t.charAt(this.textIndex),this.textIndex++,setTimeout(this.write.bind(this),this.speed)):(e&&(this.element.innerHTML='<a href="'+e+'">'+this.element.textContent+"</a>"),setTimeout(this.backspace.bind(this),this.keepTime))}backspace(){let t=this.element.textContent,e=t.length;this.textIndex>=0?(this.element.textContent=t.substring(0,e-1),this.textIndex--,setTimeout(this.backspace.bind(this),this.speed)):(this.currentText++,this.currentText>=this.texts.length&&(this.currentText=0),this.element.style.color=this.colors[this.currentText],this.write())}}document.addEventListener("DOMContentLoaded",t=>{document.querySelectorAll(".typing-text").forEach(t=>new Typing(t))});