...
var wishlist=[];localStorage&&(null==(wishlist=JSON.parse(window.localStorage.getItem("wishlist")))&&(wishlist=[]));eventListener("click",".conversation .checkboxen label",e=>{let t=e.target.closest("label");if(t.classList.contains("chat"));else{let e=t.getAttribute("data-target");document.querySelectorAll(".step").forEach(e=>e.style.display="none"),document.querySelector("."+e).style.display="block"}}),eventListener("click",".button-bottom > button",e=>{e.preventDefault();let t=e.target.closest("button");t.classList.toggle("added");let n=t.getAttribute("data-info");if(t.classList.contains("added")){n&&(console.log(wishlist),wishlist.push(n)),t.children.forEach(e=>e.innerHTML=t.getAttribute("data-add")),t.parentNode.insertAdjacentHTML("beforeend",'<div class="arrow-box shortModal" style="display: none;"><strong>'+n+"</strong>Zur Wunschliste hinzugefügt</div>");var a=next(t,".arrow-box");setTimeout(e=>{fadeIn(a,300)},450),timeoutID=window.setTimeout((function(){var e=document.querySelectorAll(".button-bottom .arrow-box");setTimeout(t=>{e.forEach(e=>{fadeOut(e,300)})},400)}),4e3)}else n&&wishlist.splice(wishlist.indexOf(n),1),t.children.forEach(e=>e.innerHTML=t.getAttribute("data-base")),document.querySelector("#wishlist .numberOfElements").innerHTML=wishlist.length;if((void 0!==wishlist||null!=wishlist)&&wishlist.length>0){console.log("wishlist: "+wishlist),window.localStorage.setItem("wishlist",JSON.stringify(wishlist)),document.querySelector("#wishlist .numberOfElements").innerHTML=wishlist.length;var s=document.querySelector(".modal.wishlist .wish-list");s.innerHTML="",wishlist.forEach(e=>{s.parentNode.insertAdjacentHTML("beforeend","<li>"+e+'<span class="sl sl-close sl-before relative"></span></li>')}),setCookie("wishlist","1",90),slideDown(document.querySelector("wishlist"),300)}else window.localStorage.removeItem("wishlist"),document.cookie="cookiename='wishlist' ; expires = Thu, 01 Jan 1970 00:00:00 GMT",slideUp(document.querySelector("#wishlist"),300)}),eventListener("click",".js-form .select-selected",e=>{let t=e.target.closest(".group"),n=window.next(t,".select-hide");n&&(n.classList.toggle("active"),n.classList.contains("active")?slideDown(n,300):slideUp(n,300))}),eventListener("click",".select-items .item",e=>{let t=e.target.closest(".item"),n=t.closest(".customSelect");n.querySelector("label").classList.add("fixed");let a=n.querySelector(".select-selected");a.innerHTML=t.textContent,a.classList.remove("active");let s=t.closest(".select-hide");slideUp(s,300,()=>{s.classList.remove("active")})}),eventListener("change",".js-form input",e=>{let t=e.target.closest(".group");t&&(t=t.querySelector("label"),t.classList.add("fixed"))});const uploadButton=document.getElementById("fileButton"),fileInfo=document.getElementById("fileInfo"),realInput=document.getElementById("realFile");null!=uploadButton&&null!=realInput&&(uploadButton.addEventListener("click",(function(){realInput.click()})),realInput.addEventListener("change",(function(){const e=realInput.value.split(/\\|\//).pop(),t=e.length>20?e.substr(e.length-20):e;fileInfo.innerHTML=t})));var sendForm=function(e){var t,n=new FormData(e),a=e.querySelector('[name="type"]').value,s=null,i=null;if(null!==a){switch(a){case"0":s={type:a,path:JSON.parse(n.get("path")),email:n.get("email")};break;case"1":if(e.classList.contains("form-1")){console.log("Rufnummern");var l=new Array;$('.customCheckbox input[name="checkboxNumbers"]:checked').length>0&&$('.customCheckbox input[name="checkboxNumbers"]:checked').each((function(){l.push($(this).val())})),s={type:a,register_phonenumbers:l,email:n.get("email"),name:n.get("name"),company:n.get("company")},t="0800 Rufnummern Reservierung",i=s.company}else s={type:a,path:JSON.parse(n.get("path")),issue:n.get("issue"),name:n.get("name"),email:n.get("email")},t="Einfaches Kontaktforumlar",i=s.email;break;case"2":var o="";o=$(e).find("#issue").hasClass("select-selected")?$("#issue").text():$(e).find('[name="issue"]').val();var r,c=[];$(e).hasClass("calcForm")?(r=n.get("employees"),$(e).find("input[name='functions']:checked").each((function(){c.push($(this).val())}))):r=$("#employees").text(),$(e).hasClass("wishlistContactForm")&&window.localStorage&&(c=JSON.parse(window.localStorage.getItem("wishlist"))),s={type:a,path:JSON.parse(n.get("path")),employees:r,issue:o,name:n.get("name"),position:$("#position").text(),company:n.get("company"),phonenumber:n.get("phonenumber"),email:n.get("email"),agents:n.get("agents"),business:n.get("business"),functions:c},n.has("newsletter")&&(s.newsletter=!!$(e).find("[name=newsletter]").attr("checked")),t=$(e).hasClass("calcForm")?"Preis Modal":"Kontaktformular",t=$(e).hasClass("wishlistContactForm")?"Wunschthemen":"Kontaktformular",i=s.company;break;case"3":n.set("type",a),n.set("path",JSON.parse($(e).find('[name="path"]').val())),n.set("issue",$(e).find('[name="issue"]').val()),n.set("name",$(e).find('[name="name"]').val()),n.set("phonenumber",$(e).find('[name="phonenumber"]').val()),n.set("email",$(e).find('[name="email"]').val()),n.set("text",$(e).find('[name="text"]').val()),n.set("file",document.getElementById("realFile").files[0]);break;case"4":n.set("type",a),n.set("path",JSON.parse($(e).find('[name="path"]').val())),n.set("issue",$(e).find('[name="issue"]').val()),n.set("name",$(e).find('[name="name"] ').val()),n.set("url",$(e).find('[name="url"] ').val());break;case"5":n.set("type",a),n.set("email",$(e).find('[name="email"]').val()),n.set("position",$(e).find('[name="position"] ').val());break;case"6":n.set("type",a),n.set("issue",$(e).find('[name="issue"]').val()),n.set("participant_email",$(e).find('[name="participant_email"]').val()),n.set("participant_name",$(e).find('[name="participant_name"]').val()),n.set("participation","ja"==$(e).find('[name="participation"]:checked').val()),"ja"==$(e).find('[name="participation_partner"]:checked').val()?n.set("partner_name",$(e).find('[name="partner_name"]').val()):n.set("partner_name"," ");break;default:return}$(e).find(".error").removeClass("error"),$(e).find(".submit").attr("disabled",!0),"1"==a||"2"==a?$.ajax({method:"POST",url:$(e).hasClass("form-1")?"https://connect.callone.io/backend/phonenumbers.php":"https://connect.callone.io/backend/contact.php",data:s,dataType:"json",success:function(n){n.error?(console.log(n.error),$(e).find("[name="+n.error+"]").addClass("error"),$(e).find(".submit").removeAttr("disabled")):($(e).hide(),$(e).next(".formSuccess").fadeIn(),dataLayer.push({_event:"formSubmit",event:"formSubmit"}),3!=a&&4!=a&&dataLayer.push({_event:"Lead",event:"Lead",eventCategory:"Lead",eventAction:t,eventLabel:i}),window._fbq=window._fbq||[],window._fbq.push(["track","6018846817861",{value:"0.00",currency:"EUR"}]))},error:function(t){console.log(t.error),$(e).hide(),$(e).next(".formFail").fadeIn()},fail:function(t){console.log(t),$(e).hide(),$(e).next(".formFail").fadeIn()}}):$.ajax({url:"https://connect.callone.io/backend/contact.php",cache:!1,type:"POST",data:n,dataType:"json",processData:!1,contentType:!1,success:function(t){t.error?(console.log(t.error),$(e).find("#"+t.error).addClass("error"),$(e).find(".submit").removeAttr("disabled")):($(e).hide(),"nein"==$(e).find('[name="participation"]:checked').val()?($(".animation-3::after").css("bottom","6px"),$(".animation-4::after").css("bottom","6px"),$(e).parent().find(".formSuccess.absage").fadeIn()):"ja"==$(e).find('[name="participation"]:checked').val()?($(".animation-3::after").css("bottom","6px"),$(".animation-4::after").css("bottom","6px"),$(e).next(".formSuccess.zusage").fadeIn()):$(e).next(".formSuccess").fadeIn())},error:function(e){console.log(e.error)},fail:function(e){console.log(e)}})}};
