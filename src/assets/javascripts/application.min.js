...
class CountingNumbers{constructor(t){this.element=t,this.speed=1e3,this.suffix=this.element.dataset.suffix||"",this.start=parseInt(this.element.dataset.start),this.end=parseInt(this.element.dataset.end),this.current=this.start,this.step=this.calculateStep(),this.running=!1,respondToVisibility(this.element,t=>{this.reset(),this.isInView()}),window.addEventListener("scroll",this.isInView.bind(this))}isInView(){this.position=this.element.getBoundingClientRect(),this.position.top>=0&&this.position.bottom<=window.innerHeight?this.running||(this.update(),this.count(),this.running=!0):this.reset()}reset(){this.running=!1,this.current=this.start,this.update()}calculateStep(){return Math.abs(this.start-this.end)/(this.speed/30)}count(){this.start<this.end?(this.current+=this.step,this.current>this.end&&(this.current=this.end)):(this.current-=this.step,this.current<this.end&&(this.current=this.end)),this.update(),this.current!=this.end?setTimeout(this.count.bind(this),1e3/30):this.running=!1}update(){this.element.textContent=Math.round(this.current)+this.suffix}}const numbers=document.querySelectorAll(".counting-number");numbers.forEach(t=>{new CountingNumbers(t)});
