...
const selectBoxes=document.querySelectorAll("[data-calloneSelect]");selectValue=e=>{let t=e.currentTarget,l=t;for(;(l=l.parentNode)&&!l.classList.contains("callone-select"););let c=l.querySelector(".callone-select__selected");c.innerHTML=t.textContent;let a=document.createElement("div");a.classList.add("callone-select__clear"),c.appendChild(a),a.addEventListener("click",deselectValue),l.querySelector(".callone-select__label").classList.add("callone-select__label--active"),l.querySelector("select").value=t.dataset.value},deselectValue=e=>{e.stopPropagation();let t=e.currentTarget;for(;(t=t.parentNode)&&!t.classList.contains("callone-select"););for(var l=t.querySelector("select").options,c=0;c<l.length;c++)l[c].selected=!1;t.querySelector(".callone-select__selected").innerHTML="&nbsp;",t.querySelector(".callone-select__label").classList.remove("callone-select__label--active")},selectBoxes.forEach(e=>{let t=document.createElement("div");t.classList.add("callone-select"),e.parentNode.insertBefore(t,e),t.appendChild(e);let l=document.createElement("div");l.classList.add("callone-select__label"),l.innerHTML=e.options[0].text,t.appendChild(l);let c=document.createElement("div");c.classList.add("callone-select__selected"),c.innerHTML="&nbsp;",t.appendChild(c);let a=document.createElement("ul");a.classList.add("callone-select__options");for(let t=1;t<e.options.length;t++){let l=e.options[t],c=document.createElement("li");c.dataset.value=l.getAttribute("value"),c.classList.add("callone-select__option"),c.innerHTML=l.text,a.appendChild(c),c.addEventListener("click",selectValue)}t.appendChild(a),t.addEventListener("click",e=>{e.stopPropagation(),t.classList.toggle("callone-select--open")})}),window.addEventListener("click",e=>{document.querySelectorAll(".callone-select").forEach(t=>{t!==e.target.parentNode&&e.target.parentNode&&t!==e.target.parentNode.parentNode&&t.classList.remove("callone-select--open")})});
class stepSlider{constructor(e){this.dragging=!1,this.slider=e,this.handle=this.slider.querySelector(".step-slider__handle"),this.mouseOffset={x:0,y:0},this.handle.addEventListener("mousedown",(e=>{this.dragging=!0;let t=this.slider.getBoundingClientRect(),s=this.getMousePosition(e);this.mouseOffset.x=s.x-t.x,this.mouseOffset.y=s.y-t.y}).bind(this)),window.addEventListener("mouseup",(e=>{this.dragging=!1}).bind(this)),window.addEventListener("mousemove",this.drag.bind(this)),this.handle.addEventListener("touchstart",(e=>{this.dragging=!0,this.mouseOffset}).bind(this)),window.addEventListener("touchend",(e=>{this.dragging=!1}).bind(this)),window.addEventListener("touchmove",this.drag.bind(this))}drag(e){if(!this.dragging)return!1;let t=this.slider.getBoundingClientRect(),s=this.getMousePosition(e);s.x+=this.mouseOffset.x,s.y+=this.mouseOffset.y,console.log(t,s,this.mouseOffset);let i=100/t.width*(s.x-t.x);i<0&&(i=0),i>100&&(i=100),this.handle.style.left=i+"%"}getMousePosition(e){let t={x:e.pageX,y:e.pageY};if("touchmove"==e.type){var s=void 0===e.originalEvent?e:e.originalEvent;let i=s.touches[0]||s.changedTouches[0];t.x=i.pageX,t.y=i.pageY}return t}}let stepSliders=document.querySelectorAll(".step-slider");stepSliders.forEach(e=>{new stepSlider(e)});